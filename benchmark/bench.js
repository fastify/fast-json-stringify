...
const BENCH_THREAD_PATH_WITH_AJV = path.join(__dirname, 'bench-thread-with-ajv.js')
...
async function runBenchmarkWithAjv (benchmark) {
  const worker = new Worker(BENCH_THREAD_PATH_WITH_AJV, { workerData: benchmark })
...
async function runBenchmarks () {
  ...
  maxNameLength += 'with ajv'.length

  for (const benchmark of benchmarks) {
    const benchmarkName = benchmark.name
    const benchmarkNameWithAjv = benchmark.name + ' with ajv'

    const resultMessage = await runBenchmark({
      ...benchmark,
      name: benchmarkName.padEnd(maxNameLength, '.')
    })
    console.log(resultMessage)

    const resultWithAjvMessage = await runBenchmarkWithAjv({
      ...benchmark,
      name: benchmarkNameWithAjv.padEnd(maxNameLength, '.')
    })
    console.log(resultWithAjvMessage)
  }
}
