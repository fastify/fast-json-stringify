...
class Compiler {
  constructor (options = {}) {
    ...
    this.validatorSchemasIds = new Set()
    ...
    if (options.schema) {
      for (const key of Object.keys(options.schema)) {
        ...
        this.refResolver.addSchema(options.schema[key], key)
      }
    }

    this.rootSchemaId = null
  }

  compileSchemaSerializer (schema, mode) {
    this.rootSchemaId = schema.$id || randomUUID()

    isValidSchema(schema)
    this.refResolver.addSchema(schema, this.rootSchemaId)

    const location = new Location(schema, this.rootSchemaId)
    ...
    for (const schemaId of this.validatorSchemasIds) {
      const schema = this.refResolver.getSchema(schemaId)
      this.validator.addSchema(schema, schemaId)

      const dependencies = this.refResolver.getSchemaDependencies(schemaId)
    ...
    if (mode === 'standalone') {
      // lazy load
      const isValidatorUsed = this.validatorSchemasIds.size > 0
      const buildStandaloneCode = require('./lib/standalone')
      return buildStandaloneCode(this.options, this.validator, isValidatorUsed, contextFunctionCode)
    }

    /* eslint no-new-func: "off" */
    const contextFunc = new Function('validator', 'serializer', contextFunctionCode)
    return contextFunc(this.validator, this.serializer)
  }
  ...
  buildValue (location, input) {
    ...
    if (schema.if && schema.then) {
      return this.addIfThenElse(location, input)
    }

    if (schema.allOf) {
      this.mergeAllOfSchema(location, schema, clone(schema))
      schema = location.schema
    }
    ...
    if (type === undefined && (schema.anyOf || schema.oneOf)) {
      this.validatorSchemasIds.add(location.getSchemaId())
  ...
  buildMultiTypeSerializer (location, input) {
    ...
    types.forEach((type, index) => {
      ...
      const nestedResult = this.buildSingleTypeSerializer(location, input)
  ...
  buildObject (location) {
    ...
    if (schemaRef.startsWith(this.rootSchemaId)) {
      schemaRef = schemaRef.replace(this.rootSchemaId, '')
    }
  ...
  buildInnerObject (location) {
    ...
    Object.keys(schema.properties || {}).forEach((key) => {
      ...
      if (propertyLocation.schema.$ref) {
        propertyLocation = this.resolveRef(location, propertyLocation.schema.$ref)
      }
    ...
    if (schema.patternProperties || schema.additionalProperties) {
      code += this.buildExtraObjectPropertiesSerializer(location)
    }

    return code
  }

  buildExtraObjectPropertiesSerializer (location) {
    const schema = location.schema
      ...
      for (const propertyKey in patternPropertiesSchema) {
        ...
        code += `
          if (/${propertyKey.replace(/\\*\//g, '\\/')}/.test(key)) {
            ...
            ${this.buildValue(propertyLocation, 'value')}
            continue
          }
        `
      }
  ...
  buildArray (location) {
    ...
    if (this.largeArrayMechanism !== 'default') {
      if (this.largeArrayMechanism === 'json-stringify') {
        ...
        throw new Error(`Unsupported large array mechanism ${this.largeArrayMechanism}`)
      }
    }
    ...
    if (Array.isArray(itemsSchema)) {
      for (let i = 0; i < itemsSchema.length; i++) {
        ...
        const tmpRes = this.buildValue(itemsLocation.getPropertyLocation(i), `obj[${i}]`)
        functionCode += `
          if (${i} < arrayLength) {
  ...
  addIfThenElse (location, input) {
    ...
    return `
      if (validator.validate("${ifSchemaRef}", ${input})) {
        ${this.buildValue(thenLocation, input)}
      } else {
        ${this.buildValue(elseLocation, input)}
      }
    `
  }
  ...
  resolveRef (location, ref) {
    ...
    if (schema.$ref !== undefined) {
      return this.resolveRef(newLocation, schema.$ref)
    }
...
function compileSchemaSerializer (schema, options = {}) {
  ...
  return compiler.compileSchemaSerializer(schema, options.mode)
}

module.exports = compileSchemaSerializer
